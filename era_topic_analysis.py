...
#
# Seven years of content from a single channel. Did it actually change — or just feel like it did?
#
# This notebook answers that question empirically. Starting from nothing but transcript embeddings and upload dates, it automatically segments the channel's history into content eras, then characterises what made each one distinct.
#
# **Disclaimer:**
# I am not a data scientist, a statisician, or anything of the sort. I just like poking data. The things presented here should be taken with a grain of salt.
# Most of the code and writing here was generated with the assistance of AI. There will almost certainly be flaws in both methodology and code. Ones I lack both the knowledge and willpower to find. But hey, pretty graphs.
...
#
# Each video is represented as a high-dimensional embedding — a vector that captures the semantic content of its transcript. We compress these into a lower-dimensional space with PCA, then use the **matrix profile** to measure, for every window of consecutive videos, how anomalous that window looks compared to the rest of the channel's history.
#
# This anomaly signal is smoothed and fed into three independent change-detection algorithms (EMA crossover, Bollinger Band breakout, and CUSUM). Where all three agree a shift occurred, we have high confidence. **CUSUM** drives the final segmentation — it accumulates evidence for a sustained shift before firing, which makes it robust to noise.
#
# ### Step 2 — What Topics Exist, and When? BERTTopic Cluster Heatmap
#
# Before drilling into words, we get the big picture: which broad **semantic topic clusters** dominate each era? BERTTopic groups videos by embedding similarity using UMAP + HDBSCAN, then labels each cluster with its most representative nouns. The heatmap shows what proportion of each era's videos fell into each cluster, sorted by total volume so the channel's biggest recurring themes sit at the top.
#
# Noun extraction (POS tagging, NER stripping of person and organisation names, lemmatisation) was run once in a separate notebook and cached to `noun_cache.csv` — both BERTTopic and the log-odds step below draw from this shared cache.
#
# ### Step 3 — What Words Were Distinctive? Log-Odds Ratio
#
# For each era, we find the nouns that are statistically *over-represented* compared to all other eras combined. This is explicitly contrastive — a word only ranks high if it appears *unusually often* in that era relative to the channel's overall history.
#
# ### Step 4 — Where Did the Channel Move? Embedding Dimension Attribution
#
# The channel's content lives in a high-dimensional embedding space. PCA finds the independent axes of variation — the directions along which videos differ most from each other. By tracking how each era's centroid moved through this space, we can identify which axes captured the real shifts.
#
# Three visualisations make these abstract axes interpretable:
# - **Radar chart** — each era as a polygon; spokes where polygons diverge = axes that distinguish eras
# - **Annotated 2D scatter** — videos on the two most-changed axes, extreme videos labelled so you can read off what each axis means
# - **Word clouds at each pole** — vocabulary of the high vs low end of each axis, showing the semantic content that defines each direction
...
# ## Config
#
# All tuneable parameters live here. The channel and content type are the first things to change if you want to run this on a different creator.
#
# **Channel / content:**
...
#   This is the primary tuning lever: wider = fewer, broader eras. At 2.7 vids/week,   35 ≈ 3 months. Try 20–50.
...
#
# **NLP:**
# - `LOG_ODDS_MIN_COUNT` — minimum era appearances for a noun to enter the log-odds ranking. Filters out one-off words that score high by chance.
#
# **Visualisation:**
...
# ## Load Data
#
# Each video is loaded with two things: its transcript text, and a numerical embedding — a vector of 384 numbers that represents the overall meaning of that video's content. These embeddings were pre-computed using a sentence transformer model trained on a large corpus of English text.
...
print(
    f"PCA explained variance ({PCA_DIMS} dims): {pca.explained_variance_ratio_.sum():.1%}"
)
...
#   parallel. A boundary fires when the short-term trend crosses the long-term   baseline and stays crossed. Because the slow EMA requires sustained movement to   shift, individual spikes don't trigger it.
#
# - **Bollinger Band Breakout** — computes a rolling mean ± N standard deviations.
#   A boundary fires when the signal escapes this band *and stays outside* for a   confirmation window. Requires both magnitude and duration.
#
# - **CUSUM (Cumulative Sum)** — accumulates evidence for an upward or downward
#   shift separately, and only fires when it exceeds a threshold, then resets.   Originally developed for industrial quality control, it's designed to answer   exactly our question: *has the mean of this process sustainably shifted?*
#
...
# %% [markdown]
# ## Step 2 — What Topics Exist, and When?
#
# Before drilling into specific words, we get the big picture. Each video's transcript was grouped into broad semantic clusters using BERTTopic — an algorithm that finds videos talking about similar things and labels each group with its most representative words. The heatmap below shows what share of each era's videos fell into each topic cluster, with the channel's biggest recurring themes at the top.
...
ax.set_title(
    "Topic prevalence by era", fontsize=14, fontweight="bold", pad=16, loc="left"
)
ax.text(
    0,
    1.02,
...
# %% [markdown]
# ## Step 3 — What Words Were Distinctive to Each Era?
#
# For this to work, raw transcripts need cleaning first. Every word in every transcript was run through a language model that strips out noise: only nouns are kept (verbs and filler words appear regardless of topic and add nothing), names of people and organisations are removed (they appear everywhere and tell us nothing about subject matter), and words are reduced to their base form so "relationships" and "relationship" count as the same thing.
#
# With that done, we can ask: which words appear *unusually often* in one era compared to all the others?

# %%
_cache = pd.read_csv("noun_cache.csv").set_index("video_id")
df["nouns"] = df["video_id"].map(
    lambda vid: _cache.loc[vid, "nouns"].split() if vid in _cache.index else []
)
print(
    f"Loaded nouns for {df['nouns'].apply(bool).sum()} / {len(df)} videos from cache."
)

# %% [markdown]
# ### Log-Odds Ratio
#
# With clean noun lists per video, we can now measure how distinctive each word is to each era. The **log-odds ratio** (Monroe et al. 2008) asks:
#
...
#
# The key advantage over TF-IDF or plain frequency: scores are explicitly *contrastive* against the rest of the channel's history, not just against other words in the era.
...
plt.suptitle(
    "Log-Odds Ratio: Nouns Most Distinctive to Each Era\n"
    "(person names and organisations removed)",
    fontsize=12,
)
plt.tight_layout()
...
for era_num, scores in enumerate(era_lo):
    ...
    print(
        f'Era {era_num+1}  ({df.iloc[s]["published_at"].date()} -> {df.iloc[e-1]["published_at"].date()})'
    )
    print(f'  {", ".join(top10)}\n')
...
#
# Log-odds tells us *which words* changed. This method tells us *which directions in content space* the channel moved along — and by how much.
#
# ### Background: what is a PCA dimension?
#
# Each video's transcript embedding is a point in 384-dimensional space. PCA finds the independent axes of greatest variation — the directions along which videos differ from each other most. PC1 explains the most variance, PC2 the next most independent amount, and so on.
#
# By computing the centroid (average position) of each era in this space, we can measure which axes the channel moved along over time. A large centroid shift along PC3 means the channel moved in whatever direction PC3 represents — we can figure out what that is by reading the videos at each extreme.
#
# ### Three visualisations
#
# **A. Era Fingerprint Radar Chart**
# Each spoke is a PCA dimension; each coloured polygon is an era. Spokes where all polygons cluster together = dimensions that were consistent across history. Spokes where they diverge = the directions the channel actually moved.
#
# **B. Annotated 2D Scatter**
# Every video projected onto the two dimensions that changed most across all era transitions. Extreme videos are labelled — reading those titles is how you interpret what each axis represents in human terms.
#
# **C. Word Clouds at Each Pole**
# For each key dimension, we split all videos into a "high" and "low" half and run log-odds between them. The resulting word clouds show the vocabulary that defines each end of the axis — together with the labelled titles, this gives you a complete semantic picture of what each dimension captures.

# %%
era_centroids = np.array([emb_pca[s:e].mean(axis=0) for s, e in era_slices])
...
# ### A. Era Fingerprint Radar Chart
#
# Each era is a polygon across all PCA dimensions. The further a spoke extends from the centre, the more that era's content leaned toward the positive end of that axis.
#
...
# ### B. Annotated 2D Scatter
#
# Every video projected onto the two PCA dimensions that changed most across all era transitions. Colour = era. Stars = era centroids. Arrows connect centroids chronologically, showing the channel's trajectory through content space.
#
# Videos at the extremes of each axis are labelled — reading those titles is how you figure out what each axis actually represents. For example, if the high end of PC2 is all mental health videos and the low end is all gaming content, PC2 is roughly a "mental health vs gaming" axis.
#
# Label colour key:
...
for era_num, (s, e) in enumerate(era_slices):
    ax.scatter(
        emb_pca[s:e, dim_x],
        emb_pca[s:e, dim_y],
        color=palette[era_num],
        alpha=0.4,
        s=18,
        label=f'Era {era_num+1} ({df.iloc[s]["published_at"].strftime("%Y-%m")})',
    )
    ax.scatter(
        era_centroids[era_num, dim_x],
        era_centroids[era_num, dim_y],
        color=palette[era_num],
        s=160,
        marker="*",
        edgecolors="k",
...
# ### C. Word Clouds at Each Pole
#
# The scatter plot tells you *which videos* sit at the extremes of each axis. The word clouds tell you *what those videos are talking about*.
#
# For each of the two most-changed PCA dimensions, we split all videos into a "high" half (above median on that axis) and a "low" half. We run log-odds between the two halves — the same method as Method 1, but now comparing the two ends of an axis rather than comparing eras.
#
# - **Blue cloud (left)** = vocabulary of the LOW end of the axis
# - **Red cloud (right)** = vocabulary of the HIGH end of the axis
#
# The titles show *what the videos are*; the word clouds show *what they say*. Together they give you a complete semantic interpretation of each axis.
...
for dim in total_change.argsort()[::-1][:TOP_DIMS_FOR_WC]:
    ...
    print(
        f'  LOW end  → {", ".join(sorted(wc_low, key=wc_low.get, reverse=True)[:10])}'
    )
    print(
        f'  HIGH end → {", ".join(sorted(wc_high, key=wc_high.get, reverse=True)[:10])}'
    )
...
#
# For each pair of adjacent eras, this bar chart shows how much the centroid moved along each PCA dimension. Red bars = the two dimensions that changed most at that specific transition.
#
# Use this to identify *which transition* drove the biggest shift and *on which axis*. Cross-reference with the word clouds to understand what that shift meant in content terms.
...
#
# A plain-text printout of the videos at the high and low ends of the three most-changed PCA dimensions. Each entry shows the era, publish date, and title.
#
# This is the fastest way to answer "what does this dimension represent?" — if the high-end videos are all about ADHD and the low-end videos are all about relationship dynamics, you know exactly what the axis is capturing.
...
for dim in total_change.argsort()[::-1][:3]:
    scores_dim = emb_pca[:, dim]
    print(
        f"PC{dim+1}  ({pca.explained_variance_ratio_[dim]:.1%} of variance  |  "
    ...
    for idx in top_idx:
        print(
            f'    [Era {int(df["era"].iloc[idx])+1}]  {df["published_at"].iloc[idx].date()}'
